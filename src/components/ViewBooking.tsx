...
function ViewBooking() {
  ...
  const handleDeleteBooking = async (bookingId: string) => {
    MySwal.fire({
      title: <div className={language === 'TH' ? 'font-anuphan font-black text-[#7E4300] text-lg' : 'font-tiroTamil text-[#7E4300] text-lg'}>{t.areYouSureDelete}</div>,
      html: <div className={language === 'TH' ? 'font-anuphan font-black text-[#6B4423] text-sm' : 'font-tiroTamil text-[#6B4423] text-sm'}>{t.cantChangeDecision}</div>,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#7E4300",
      cancelButtonColor: "#7E4300",
      confirmButtonText: <span className={language === 'TH' ? 'font-anuphan font-black' : 'font-tiroTamil'}>{t.yesDeleteIt}</span>,
      cancelButtonText: <span className={language === 'TH' ? 'font-anuphan font-black' : 'font-tiroTamil'}>{t.noKeepIt}</span>
    }).then(async (result) => {
      if (result.isConfirmed) {
        try {
          ...
          MySwal.fire({
            title: <div className={language === 'TH' ? 'font-anuphan font-black text-[#7E4300] text-lg' : 'font-tiroTamil text-[#7E4300] text-lg'}>{t.bookingCancelled}</div>,
            html: <div className={language === 'TH' ? 'font-anuphan font-black text-[#6B4423] text-sm' : 'font-tiroTamil text-[#6B4423] text-sm'}>{t.bookingDeleted}</div>,
            icon: "success",
            confirmButtonColor: "#7E4300",
          ...
          MySwal.fire({
            title: <div className={language === 'TH' ? 'font-anuphan font-black text-[#7E4300] text-lg' : 'font-tiroTamil text-[#7E4300] text-lg'}>{t.error}</div>,
            html: <div className={language === 'TH' ? 'font-anuphan font-black text-[#6B4423] text-sm' : 'font-tiroTamil text-[#6B4423] text-sm'}>{t.failedToCancel}</div>,
            icon: "error",
            confirmButtonColor: "#7E4300",
            confirmButtonText: <span className="font-tiroTamil">OK</span>
          });
    ...
    <div className="m-0 w-[360px] min-h-[90vh] bg-white flex flex-col items-center justify-start px-4 pt-2 pb-4">
      {loading ? (
        <div className="m-0 w-[360px] min-h-[90vh] bg-white flex flex-col relative">
          <p className={language === 'TH' ? 'text-[#7E4300] text-center mt-10 font-anuphan font-black' : 'text-[#7E4300] text-center mt-10 font-tiroTamil'}>{t.loadingBookings}</p>
        </div>
        ...
        <div className="flex flex-col items-center gap-4 w-full">
          ...
          <div className="flex items-center justify-center w-full relative mt-2">
            ...
            <div className={language === 'TH' ? 'text-[#7E4300] text-xl font-anuphan font-black' : 'text-[#7E4300] text-xl font-tiroTamil'}>{t.yourBookings}</div>
          </div>
                ...
                <div className="flex justify-between items-start text-left relative">
                  ...
                  <div className="flex flex-col gap-1 text-left">
                    ...
                    <div className="text-[#818181] font-medium text-sm mb-1 flex items-center gap-1">
                      ...
                      <span className={language === 'TH' ? 'animate-pulse font-anuphan font-black' : 'animate-pulse'}>{t.pending}</span>
                    </div>
                    <div className={language === 'TH' ? 'text-[#6B4423] font-medium text-left text-sm font-anuphan font-black' : 'text-[#6B4423] font-medium text-left text-sm'}>
          ...
          <button className={language === 'TH' ? 'mt-2 !bg-[#DCA900] text-white text-xs font-anuphan font-black self-center cursor-pointer py-1 px-2 rounded border-none hover:scale-110 hover:opacity-80 transition-all' : 'mt-2 !bg-[#DCA900] text-white text-xs font-tiroTamil self-center cursor-pointer py-1 px-2 rounded border-none hover:scale-110 hover:opacity-80 transition-all'} onClick={() => navigate('/makebooking')}>{t.makeBooking}</button>
        </div>
      ) : (
        <div className="flex flex-col items-center gap-4 w-full">
          {/* Header with back button and title */}
          ...
          <div className="flex flex-col items-center gap-4 justify-start p-4">
            <div className={language === 'TH' ? 'text-[#7E4300] text-lg font-anuphan font-black' : 'text-[#7E4300] text-lg font-tiroTamil'}>{t.noBookings}</div>

            {/* Make a booking button */}
            <button className={language === 'TH' ? 'mt-2 !bg-[#DCA900] text-white text-xs font-anuphan font-black self-center cursor-pointer py-1 px-2 rounded border-none hover:scale-110 hover:opacity-80 transition-all disabled:opacity-50 disabled:cursor-not-allowed' : 'mt-2 !bg-[#DCA900] text-white text-xs font-tiroTamil self-center cursor-pointer py-1 px-2 rounded border-none hover:scale-110 hover:opacity-80 transition-all disabled:opacity-50 disabled:cursor-not-allowed'} onClick={() => navigate('/makebooking')}>{t.makeBooking}</button>
          </div>
        </div>
      )}
    </div>
