    ...
    <div className="w-[360px] min-h-screen bg-white flex flex-col">
      {loading ? (
        <div className="w-[360px] min-h-screen bg-white flex flex-col relative">
          <p className={language === 'TH' ? 'text-[#7E4300] text-center mt-10 font-anuphan font-black' : 'text-[#7E4300] text-center mt-10 font-tiroTamil'}>{t.loadingHistory}</p>
        </div>
      ) : (
        <div className="flex flex-col items-center gap-2 w-full">
          ...
          <div className="flex items-center justify-center w-full relative px-4 py-3">
            <img 
              ...
              onClick={() => navigate('/')}
            />
            <div className={language === 'TH' ? 'text-2xl font-anuphan font-black text-[#7E4300]' : 'text-2xl font-tiroTamil text-[#7E4300]'}>{t.history}</div>
          </div>
          ...
          <div className="flex flex-col gap-4 w-full px-4 mt-4">
            {bookings.length > 0 ? (
                  ...
                  <div key={booking.id} className="rounded-xl p-4 bg-white border-2 border-[#6B4423] relative">
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex flex-col gap-1 text-left">
                        <div className={language === 'TH' ? 'text-[#6B4423] font-medium text-sm text-left font-anuphan font-black' : 'text-[#6B4423] font-medium text-sm text-left'}>
                      ...
                      <div className="flex flex-col items-end gap-1">
                        <div className={`text-xl font-medium ${getStatusColor(status)} ${language === 'TH' ? 'font-anuphan font-black' : ''}`}>
              ...
              <div className="flex justify-center items-center py-0">
                <div className={language === 'TH' ? 'text-[#7E4300] text-lg font-anuphan font-black' : 'text-[#7E4300] text-lg font-tiroTamil'}>{t.noHistory}</div>
              </div>
            )}
          </div>
        </div>
